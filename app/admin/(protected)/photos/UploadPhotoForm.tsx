...
export default function UploadPhotoForm({ categories }: { categories: Category[] }) {
  ...
  const [state, formAction, pending] = useActionState(
    async (prev: { error?: string } | undefined, formData: FormData) => {
      if (cropData) {
        formData.set("cropX", String(Math.round(cropData.x)));
        formData.set("cropY", String(Math.round(cropData.y)));
        formData.set("cropW", String(Math.round(cropData.width)));
        formData.set("cropH", String(Math.round(cropData.height)));
      }
  ...
  return (
    <form
      ref={formRef}
      action={formAction}
      className="space-y-6 rounded-sm border border-border bg-background-soft p-6"
    >
      ...
      {file && (
        <ImageCropper file={file} aspectRatio={4 / 5} onCrop={setCropData} />
      )}
